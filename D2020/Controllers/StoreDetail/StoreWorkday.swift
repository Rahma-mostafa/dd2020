...
class StoreWorkday: BaseController {
    ...
    static weak var instance: StoreWorkday?
    ...
    func getStoreWorkDaySelected(path: Int) -> StoreDetail.Days? {
        for item in StorePremuimController.storePrem?.days ?? [] {
            switch path {
                case 0:
                    if item.key == "sat" {
                        return item
...
extension StoreWorkday: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func setupCell(cell: StoreWorkdayCell, path: Int) {
        StorePremuimController.storePrem?.days?.forEach({ (item) in
            switch path {
                case 0:
                    if item.key == "sat" {
