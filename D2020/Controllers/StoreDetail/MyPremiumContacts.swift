...
class MyPremiumContacts: BaseController {
    ...
    static weak var instance: MyPremiumContacts?
    override func viewDidLoad() {
        super.hiddenNav = true
        super.viewDidLoad()
        setup()
        MyPremiumContacts.instance = self
    }
    func setup() {
...
extension MyPremiumContacts: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        switch indexPath.row {
            case 0:
                openUrl(text: MyStorePremuimController.storePrem?.facebook)
            case 1:
                openUrl(text: MyStorePremuimController.storePrem?.facebook)

            case 2:
                let url  = URL(string: "whatsapp://send?phone=\(MyStorePremuimController.storePrem?.whatsapp ?? "")&abid=12354&text=D2020")
                ...
                openUrl(text: MyStorePremuimController.storePrem?.whatsapp)

            case 3:
                openUrl(text: MyStorePremuimController.storePrem?.instagram)

            case 4:
                openUrl(text: MyStorePremuimController.storePrem?.snap)

            case 5:
                openUrl(text: MyStorePremuimController.storePrem?.website)
